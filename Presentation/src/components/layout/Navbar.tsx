  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-gray-200 shadow-sm">
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-3">
        ...
        {/* Actions — anclado siempre a la derecha */}
        <div className="flex items-center gap-2 ml-auto flex-shrink-0">
          ...
          {/* Botones de sesión — el módulo de Auth los conectará */}
          {!isAuthenticated && (
            <>
          ...
          {isAuthenticated && currentUser && (
            <div className="hidden sm:flex items-center gap-2">
              ...
              {/* Botón de cerrar sesión — implementado por módulo de Auth */}
              <button
                id="btn-logout"
                className="text-sm text-red-500 border border-red-200 px-3 py-1.5 rounded-full hover:bg-red-50 transition-colors whitespace-nowrap"
              >
                Salir
              </button>
            </div>
      ...
      <nav className="bg-purple-700">
        <div className="w-full px-4 sm:px-6 lg:px-8">
          <ul className="flex items-center gap-1 text-white text-sm">
            ...
            {/* Clientes y Finanzas — solo para Administradores */}
            {isAdmin && (
              <>
                <li>
                  <button
