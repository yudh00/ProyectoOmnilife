...
interface NavbarProps {
  ...
  currentPage: "home" | "catalog" | "clients" | "finances" | "orders" | "my-orders";
  onNavigate: (page: "home" | "catalog" | "clients" | "finances" | "orders" | "my-orders") => void;
}
        ...
        <nav className="bg-purple-700">
          <div className="w-full px-4 sm:px-6 lg:px-8 overflow-x-auto">
            <ul className="flex items-center gap-1 text-white text-sm w-max min-w-full">
              ...
              {/* 2.b MIS PEDIDOS: Solo para clientes (no administradores) */}
              {isAuthenticated && !isAdmin && (
                <li>
                  <button
                    onClick={() => onNavigate("my-orders")}
                    className={`flex items-center gap-1.5 px-4 py-3 transition-colors whitespace-nowrap font-medium ${
                      currentPage === "my-orders" ? "bg-purple-900" : "hover:bg-purple-800"
                    }`}
                  >
